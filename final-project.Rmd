...
zip_stops <- read_csv("zip_stops.csv")
...
leaflet() %>%
  ...
  addPolygons(data=zip_boundary, opacity = .5, fillColor = "#00a1e4", 
              ...
              popup = zip_boundary@data[["ZIPCODE"]])
```

```{r}
routes_csv <- read_csv("Bus_Route.csv")
stops_csv <- read_csv("Bus_Stop.csv")
count_routes <- routes_csv %>%
  count(RTNAME)
```
